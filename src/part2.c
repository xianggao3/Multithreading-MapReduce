...
int part2(size_t nthreads){
    ...
    //printf("%d threads found\n", NUM_FILES);//returns 500
    if(NUM_FILES<nthreads){
        nthreads=NUM_FILES;
    }
    if(nthreads==0){
    ...
    data_array=malloc((NUM_FILES)*sizeof(struct site*));
    ...
    numperthread=NUM_FILES/nthreads;
    //int numperthrea=numperthread;
    if ((NUM_FILES%nthreads)!=0){
        numperthread++;
    }
    //printf("%d\n",numperthread );
...
static void* reduce(void* v){;
  ...
  if(strcmp(QUERY_STRINGS[current_query],"B")==0){
    ...
    for(minIndex=1;minIndex<NUM_FILES;minIndex++){
    ...
    for(maxIndex=1;maxIndex<NUM_FILES;maxIndex++){

...
int part5(size_t nthreads){
    ...
    //printf("%d threads found\n", NUM_FILES5);//returns 500
    data_array3=malloc((NUM_FILES5)*sizeof(struct site*));
    if(NUM_FILES5<nthreads){
        nthreads=NUM_FILES5;
    }
    if(nthreads==0){
    ...
    numperthread3=NUM_FILES5/nthreads;
    //int numperthrea=numperthread3;
    if ((NUM_FILES5%nthreads)!=0){
        numperthread3++;
    }
    //printf("%d\n",NUM_FILES5  );
    ...
    for(tc=0;tc<nthreads;tc++){
        ...
        *baseIndex = tc*numperthread3;
        rc = pthread_create(&threads[tc], NULL, multimap5, (void*)baseIndex);//make tthreads
         if (rc){
 ...
 void* multimap5(void* base){
    //printf("%d\n",numperthread3 );
    int* da = (int *) base;
    int nUntilNum;
    for(nUntilNum=0;nUntilNum<numperthread3;nUntilNum++){
        //printf("%d\n",nUntilNum );
        if(*da+nUntilNum<NUM_FILES5){
